...
import {buildAnnotationBeats, phaseBeatId, type AnnotationBeat} from "./annotationLogic";
...
export const AnnotationCoachExplainer = () => {
...
const ExplainerWorld: React.FC<{
  ...
  return (
    <group>
      ...
      <AnnotationLayer clip={clip} shotFrame={shotFrame} contactFrame={contactFrame} activeBeatId={activeBeatId} />
...
const ExplainerOverlay: React.FC<{clip: Clip; frame: number; phase: string}> = ({clip, frame, phase}) => {
  ...
  const activeBeat = beats.find((beat) => beat.id === phaseBeatId(phase)) || beats[0];
    ...
    <>
      ...
      {isOutro ? null : (
        <div style={styles.storyPanel}>
          <div style={styles.storyKicker}>{isIntro ? phaseLabel : activeBeat.family}</div>
          <div style={styles.storySummary}>{isIntro ? "We freeze the strike, read the mechanics, then release the ball." : activeBeat.headline}</div>
          <div style={styles.storyText}>
            {isIntro ? "The viewer sees context, then measured annotations attach directly to the relevant body parts." : activeBeat.implication}
          </div>
          {isIntro ? null : (
            <div style={styles.implicationGrid}>
              <InsightLine label="Measured" text={activeBeat.label} />
              <InsightLine label="Visual" text={annotationVisualLabel(activeBeat.visual)} />
              <InsightLine label="Why" text={implication.optimizes} />
            </div>
          )}
        </div>
      )}
...
const activeBeatIdForSegment = (segmentFrame: number) => phaseBeatId(phaseForSegment(segmentFrame));
const annotationVisualLabel = (visual: AnnotationBeat["visual"]) => {
  if (visual === "distance-line") return "shortest line from foot to ball";
  if (visual === "ground-distance") return "ground measure from plant foot to ball";
  if (visual === "angle-arc") return "axis lines plus rotation arc";
  if (visual === "path-trace") return "strike-foot trail into contact";
  return "ball trajectory after release";
};
...
const AnnotationLayer: React.FC<{
  ...
  contactFrame: ShotFrame;
  activeBeatId: string;
}> = ({clip, shotFrame, contactFrame, activeBeatId}) => {
  const beats = buildAnnotationBeats({role: clip.role, score: clip.score, features: clip.features});
  ...
  if (beat.id === "contact-gap") {
    ...
    const midpoint = foot.clone().lerp(ball, 0.5).add(new THREE.Vector3(0, 0.42, 0));
    return (
      <group>
        <MeasurementLine start={foot} end={ball} color={CONTACT_COLOR} />
        <Endpoint point={foot} color={CONTACT_COLOR} />
        <Endpoint point={ball} color={CONTACT_COLOR} />
        <LabelSprite position={midpoint} text={beat.label} color={CONTACT_COLOR} />
      </group>
    );
  }
  ...
  if (beat.id === "plant-base") {
    ...
    const midpoint = start.clone().lerp(end, 0.5).add(new THREE.Vector3(0, 0.34, 0));
    return (
      <group>
        <MeasurementLine start={start} end={end} color={PLANT_COLOR} />
        <GroundRing point={start} color={PLANT_COLOR} radius={0.32} />
        <GroundRing point={end} color={CONTACT_COLOR} radius={0.42} />
        <LabelSprite position={midpoint} text={beat.label} color={PLANT_COLOR} />
      </group>
    );
  }
  ...
  if (beat.id === "hip-shoulder") {
    ...
    return (
      <group>
        <MeasurementLine start={toThree(leftShoulder)} end={toThree(rightShoulder)} color={SHOULDER_COLOR} />
        <MeasurementLine start={toThree(leftHip)} end={toThree(rightHip)} color={HIP_COLOR} />
        <AngleArc
          center={center}
        ...
        />
        <LabelSprite position={center.clone().add(new THREE.Vector3(0, 0.45, 0))} text={beat.label} color={HIP_COLOR} />
      </group>
    );
  }
  ...
  if (beat.id === "foot-path") {
    ...
    return (
      <group>
        ...
        <LabelSprite position={foot.clone().add(new THREE.Vector3(0, 0.55, 0))} text={beat.label} color={FOOT_PATH_COLOR} />
      </group>
    );
  }
  ...
  return (
    <group>
      ...
      <LabelSprite position={currentBall.clone().add(new THREE.Vector3(0, 0.68, 0))} text={beat.label} color={BALL_PATH_COLOR} />
    </group>
  );
};

const MeasurementLine: React.FC<{start: THREE.Vector3; end: THREE.Vector3; color: string}> = ({start, end, color}) => (
  <CylinderBetween start={start} end={end} radius={0.026} color={color} opacity={0.94} />
);
...
const GroundRing: React.FC<{point: THREE.Vector3; color: string; radius: number}> = ({point, color, radius}) => (
...
const AngleArc: React.FC<{
  ...
  hipA: THREE.Vector3;
  hipB: THREE.Vector3;
}> = ({center, shoulderA, shoulderB, hipA, hipB}) => {
  const points = useMemo(() => {
    ...
    return result;
  }, [center, hipA, hipB, shoulderA, shoulderB]);
...
const cameraForSegment = (clip: Clip, segmentFrame: number, shotFrame: ShotFrame, contactFrame: ShotFrame, activeBeatId: string) => {
  ...
  if (segmentFrame < CONTACT_TRIGGER_END) {
    ...
    radius = lerp(5.2, 4.45, progress);
    height = lerp(2.9, 2.55, progress);
    ...
    radius = lerp(4.8, 5.7, progress);
    height = lerp(2.75, 3.35, progress);
    ...
    radius = lerp(5.9, 8.6, progress);
    height = lerp(3.35, 4.3, progress);
