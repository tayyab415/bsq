...
const ExplainerWorld: React.FC<{
  ...
  const showAxes = segmentFrame >= 34 && segmentFrame < 118;
  ...
  return (
    <group>
      ...
      {showAxes && contactShooter ? <BodyAxes player={contactShooter} /> : null}
  ...
  return (
    <>
      ...
      <div style={styles.storyPanel}>
        ...
        <div style={styles.storySummary}>{clip.story.summary}</div>
...
const BodyAxes: React.FC<{player: Player}> = ({player}) => {
...
const activeClip = (frame: number) => {
  ...
  if (frame < 180) return {clip: data.clips[0], segmentFrame: frame - 30, phase: "standout mechanics"};
  if (frame < 330) return {clip: data.clips[1], segmentFrame: frame - 180, phase: "mechanics constraint"};
...
const frameForSegment = (clip: Clip, segmentFrame: number): ShotFrame => {
  ...
  if (segmentFrame < 118) return nearestFrame(clip, contact);
  const frameNumber = interpolate(segmentFrame, [118, 149], [contact, clip.frameWindow.end], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
...
const cameraForSegment = (clip: Clip, segmentFrame: number, shotFrame: ShotFrame, contactFrame: ShotFrame) => {
  ...
  if (segmentFrame < 34) {
    ...
    radius = lerp(40, 20, progress);
    height = lerp(18, 8, progress);
    angle = lerp(-1.15, -0.72, progress);
    target = liveTarget.clone().lerp(contactTarget, progress * 0.7);
  } else if (segmentFrame < 58) {
    const progress = eased(segmentFrame, 34, 58);
    radius = lerp(20, 5.2, progress);
    height = lerp(8, 2.35, progress);
    angle = -0.72;
  } else if (segmentFrame < 112) {
    const progress = (segmentFrame - 58) / 54;
    radius = 5.2;
    height = 2.35;
    angle = -0.72 + Math.PI * 2 * progress;
  } else if (segmentFrame < 130) {
    const progress = eased(segmentFrame, 112, 130);
    radius = lerp(5.2, 31, progress);
    height = lerp(2.35, 13, progress);
    angle = lerp(-0.72 + Math.PI * 2, -0.95, progress);
  } else {
    const progress = eased(segmentFrame, 130, 149);
    target = contactTarget.clone().lerp(liveTarget, progress);
    radius = lerp(31, 35, progress);
    height = lerp(13, 15, progress);
    angle = -0.95;
  }
