...
export const ExtendedContactSlowMoExplainer = () => {
    ...
    <>
      ...
      <div style={styles.storyPanel}>
        ...
        <div style={styles.storyText}>
          {isContactHold ? "The frame stays at contact so the plant leg, hip line, shoulder line, and foot-ball relationship can be read before release." : isSlowMo ? cue.detail : "Contact freezes first; then the shot releases in extended slow motion."}
        </div>
...
const frameForSegment = (clip: Clip, segmentFrame: number): ShotFrame => {
  ...
  if (segmentFrame < 90) return nearestFrame(clip, contact);
  const frameNumber = interpolate(segmentFrame, [90, 239], [contact, clip.frameWindow.end], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
...
const cameraForSegment = (clip: Clip, segmentFrame: number, shotFrame: ShotFrame, contactFrame: ShotFrame) => {
  ...
  } else if (segmentFrame < 90) {
    const progress = eased(segmentFrame, 30, 90);
    target = mechanicsTarget(clip, contactFrame);
    radius = lerp(5.8, 5.4, progress);
    height = lerp(2.25, 2.1, progress);
    angle = lerp(-0.72, -0.48, progress);
  } else {
    const progress = eased(segmentFrame, 90, 239);
    target = mechanicsTarget(clip, shotFrame).lerp(liveTarget, progress * 0.18);
    radius = lerp(5.4, 8.7, progress);
    height = lerp(2.1, 3.8, progress);
    angle = lerp(-0.48, 0.72, progress);
  }
