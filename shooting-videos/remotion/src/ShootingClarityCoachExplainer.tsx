...
const TEAMMATE_PLAYER_COLOR = "#9ba8a1";
const OPPOSITION_PLAYER_COLOR = "#d4ddd8";
const SHOOTER_BONE_COLOR = "#f4f6ee";
const SHOOTER_JOINT_COLOR = "#ffffff";
const SHOOTER_HALO_COLOR = "#eef6ef";
const STANDOUT_TINT = "#e6eee8";
const CONSTRAINT_TINT = "#a8b2ab";
const SHOULDER_COLOR = "#246bfe";
const HIP_COLOR = "#ff8a1f";
const PLANT_COLOR = "#7ed957";
const BALL_COLOR = "#efe0ad";
const BALL_PATH_COLOR = "#f6d32d";
const FOOT_PATH_COLOR = "#8f4dff";
const CONTACT_COLOR = "#ff3366";
...
export const ClarityCoachExplainer = () => {
    ...
    <group>
      ...
      {(shotFrame.players || []).map((player) => (
        <PlayerSkeleton
          ...
          shooterTeamCode={shooter?.teamCode}
    ...
    <>
      ...
      {isOutro ? null : (
        <div style={styles.storyPanel}>
          ...
          <div style={styles.storySummary}>{isIntro ? "We freeze the strike, read the mechanics, then release the ball." : cue.headline}</div>
          <div style={styles.storyText}>
            {isIntro ? "The shooter holds neutral. Color is reserved for measured overlays only." : cue.detail}
          </div>
        </div>
      )}
      ...
      <div style={styles.calloutGrid}>
        <MetricCard
          ...
          detail={isStandout ? "clean phase chain" : "mechanics bottleneck"}
          big
        />
        <MetricCard label="Hip-shoulder" value={separation} detail="peak rotational interval" />
        <MetricCard label="Plant base" value={formatDistance(plantLateral)} detail="lateral offset" />
        <MetricCard label="Contact gap" value={formatDistance(footGap)} detail="foot-to-ball at strike" />
      </div>
...
const activeCuesForPhase = (
  ...
  if (phase === "approach to contact") {
    return [
      {color: SHOOTER_BONE_COLOR, label: "Subject", value: "skeleton in white"},
      {color: FOOT_PATH_COLOR, label: "Strike foot", value: `${values.footSpeed} into ball`},
      {color: CONTACT_COLOR, label: "Contact point", value: "marker locks at impact"},
    ];
  }
  if (phase === "contact hold") {
    return [
      {color: CONTACT_COLOR, label: "Contact point", value: `${values.contactGap} foot-ball gap`},
      {color: PLANT_COLOR, label: "Plant base", value: `${values.plantLateral} lateral`},
      {color: SHOULDER_COLOR, label: "Shoulder axis", value: "upper-body line"},
      {color: HIP_COLOR, label: "Hip axis", value: "lower-body line"},
    ];
  }
  if (phase === "mechanics breakdown") {
    return [
      {color: SHOULDER_COLOR, label: "Shoulder axis", value: "upper-body line"},
      {color: HIP_COLOR, label: "Hip axis", value: `${values.hipSeparation} interval`},
      {color: FOOT_PATH_COLOR, label: "Foot path", value: "approach into ball"},
      {color: PLANT_COLOR, label: "Plant base", value: `${values.plantLateral} lateral`},
    ];
  }
  if (phase === "release path") {
    return [
      {color: BALL_PATH_COLOR, label: "Ball path", value: `${values.ballSpeed} exit`},
      {color: FOOT_PATH_COLOR, label: "Foot path", value: "trailing into ball"},
      {color: CONTACT_COLOR, label: "Contact point", value: `${values.contactGap} at strike`},
    ];
  }
  return [
...
const PlayerSkeleton: React.FC<{player: Player; isShooter: boolean; shooterTeamCode?: number; muted: boolean}> = ({
  player,
  isShooter,
  shooterTeamCode,
  muted,
  ...
  if (!isShooter) {
    const isOpposition = shooterTeamCode !== undefined && player.teamCode !== undefined && player.teamCode !== shooterTeamCode;
    const opacity = isOpposition ? (muted ? 0.2 : 0.34) : (muted ? 0.11 : 0.2);
    const radius = isOpposition ? 0.032 : 0.026;
    const jointRadius = isOpposition ? 0.058 : 0.046;
    const color = isOpposition ? OPPOSITION_PLAYER_COLOR : TEAMMATE_PLAYER_COLOR;
    return (
      <group>
        ...
        {Object.entries(player.parts).map(([name, point]) => (
          <mesh key={`background-joint-${name}`} position={toThree(point)}>
            <sphereGeometry args={[jointRadius, 10, 10]} />
            <meshStandardMaterial
              color={color}
              transparent
              opacity={Math.min(0.44, opacity + 0.08)}
              roughness={0.5}
            />
          </mesh>
        ))}
...
const ContactGhost: React.FC<{player: Player}> = ({player}) => (
  <group>
    {BODY_CONNECTIONS.map(([from, to]) => {
      ...
      const b = player.parts[to];
      if (!a || !b) return null;
      return <CylinderBetween key={`${from}-${to}`} start={toThree(a)} end={toThree(b)} radius={0.022} color={SHOOTER_HALO_COLOR} opacity={0.18} />;
    })}
  </group>
);
...
const styles: Record<string, React.CSSProperties> = {
  ...
  storySummary: {marginTop: 8, fontSize: 26, lineHeight: 1.12, fontWeight: 850},
  storyText: {marginTop: 10, color: "#cdd6cf", fontSize: 17, lineHeight: 1.32, fontWeight: 500},
  coachPanel: {
    position: "absolute",
    left: 54,
    top: 210,
