...
import {buildAnnotationBeats} from "./annotationLogic";
...
const SHOULDER_COLOR = "#7dc7ff";
const HIP_COLOR = "#ffbe55";
const PLANT_COLOR = "#f6dc65";
const BALL_COLOR = "#efe0ad";
const BALL_PATH_COLOR = "#f6d32d";
const FOOT_PATH_COLOR = "#9ce8ff";
const CONTACT_COLOR = "#ff2f6d";
const MEASURE_COLOR = "#f8e36a";
const MUTED_MEASURE_COLOR = "#d6dfd8";

const INTRO_FRAMES = 60;
const CLIP_FRAMES = 840;
const CONTACT_TRIGGER_END = 120;
const PLANT_MEASURE_END = 255;
const CONTACT_MEASURE_END = 390;
const MECHANICS_END = 630;
const OUTRO_START = INTRO_FRAMES + CLIP_FRAMES;

export const MeasurementFocusExplainer = () => {
  ...
  const activeBeatId = activeBeatIdForSegment(segmentFrame);
  const camera = useMemo(
    () => cameraForSegment(clip, segmentFrame, shotFrame, contactFrame, activeBeatId),
    [activeBeatId, clip, contactFrame, segmentFrame, shotFrame],
  );
  ...
  return (
    <AbsoluteFill style={styles.root}>
      <ThreeCanvas width={width} height={height}>
        ...
        <ExplainerWorld clip={clip} shotFrame={shotFrame} contactFrame={contactFrame} segmentFrame={segmentFrame} activeBeatId={activeBeatId} />
      </ThreeCanvas>
      <div style={styles.vignette} />
      <ExplainerOverlay clip={clip} frame={frame} segmentFrame={segmentFrame} phase={phase} />
    </AbsoluteFill>
  );
};
...
const ExplainerWorld: React.FC<{
  ...
  segmentFrame: number;
  activeBeatId: string;
}> = ({clip, shotFrame, contactFrame, segmentFrame, activeBeatId}) => {
  ...
  const showMechanics = segmentFrame >= CONTACT_TRIGGER_END - 18 && segmentFrame < CLIP_FRAMES;
    ...
    <group>
      ...
      {(shotFrame.players || []).map((player) => (
        <PlayerSkeleton
          ...
          muted={player.name !== shooterName}
          tint={clip.role === "standout" ? STANDOUT_TINT : CONSTRAINT_TINT}
        />
      ))}
      {ball ? <BallMesh position={ball} /> : null}
      {showMechanics && contactBall ? <ContactRings position={contactBall} /> : null}
      {showMechanics && contactBall ? <ContactPointMarker position={contactBall} /> : null}
      <MeasurementFocusLayer
        clip={clip}
        shotFrame={shotFrame}
...
const ExplainerOverlay: React.FC<{clip: Clip; frame: number; segmentFrame: number; phase: string}> = ({clip, frame, segmentFrame, phase}) => {
  ...
  const beats = buildAnnotationBeats({role: clip.role, score: clip.score, features: clip.features});
  const activeBeat = beats.find((beat) => beat.id === activeBeatIdForSegment(segmentFrame)) || beats[0];
  const focus = focusCopy(activeBeat.id, clip);
  ...
  return (
    <>
      <div style={styles.topBar}>
        <div style={styles.titleBlock}>
          ...
          <div style={styles.title}>
            {isIntro ? "Measure the contact, then release the shot" : clip.shot.player || "Shot"}
          </div>
      ...
      <div style={styles.focusPanel}>
        <div style={styles.storyKicker}>{isIntro ? "Measurement setup" : focus.kicker}</div>
        <div style={styles.storySummary}>
          {isIntro ? "One active measurement at a time." : focus.headline}
        </div>
        <div style={styles.storyText}>
          {isIntro
            ? "The shot freezes around contact, draws the actual distance or axis in the 3D scene, then releases the ball in slow motion."
            : focus.detail}
        </div>
        <div style={styles.activeMetricRow}>
          <span style={{...styles.activeMetricDot, background: focus.color}} />
          <span style={styles.activeMetricValue}>{activeBeat.label}</span>
          <span style={styles.activeMetricWhy}>{focus.read}</span>
        </div>
      </div>

      <div style={styles.metricRail}>
        <MetricCard accent={roleTint} label="Technique" value={formatScore(currentScore)} detail={isStandout ? "standout chain" : "limiting chain"} />
        <MetricCard accent={activeBeat.id === "plant-base" ? MEASURE_COLOR : undefined} label="Plant to ball" value={formatDistance(plantLateral)} detail="actual brace width" />
        <MetricCard accent={activeBeat.id === "contact-gap" ? CONTACT_COLOR : undefined} label="Foot to ball" value={formatDistance(footGap)} detail="contact spacing" />
        <MetricCard accent={activeBeat.id === "hip-shoulder" ? HIP_COLOR : undefined} label="Hip-shoulder" value={separation} detail="rotation interval" />
      </div>
    </>
  );
};
...
const activeBeatIdForSegment = (segmentFrame: number) => {
  if (segmentFrame < CONTACT_TRIGGER_END) return "foot-path";
  if (segmentFrame < PLANT_MEASURE_END) return "plant-base";
  if (segmentFrame < CONTACT_MEASURE_END) return "contact-gap";
  if (segmentFrame < MECHANICS_END) return "hip-shoulder";
  return "ball-path";
};

const focusCopy = (beatId: string, clip: Clip) => {
  ...
  const separation = clip.story.callouts[0]?.value || "-";
  if (beatId === "plant-base") {
    return {
      kicker: "Plant foot to ball",
      headline: "Brace width is drawn on the pitch.",
      detail: `The bracket grows from the planted foot to the ball: ${plant}. This shows whether the player has a stable post to rotate around.`,
      read: "distance bracket",
      color: MEASURE_COLOR,
    };
  }
  if (beatId === "contact-gap") {
    return {
      kicker: "Strike foot to ball",
      headline: "Contact spacing is measured at impact.",
      detail: `The short line connects the striking foot to the ball: ${footGap}. Smaller spacing usually means cleaner transfer into the shot.`,
      read: "contact line",
      color: CONTACT_COLOR,
    };
  }
  if (beatId === "hip-shoulder") {
    return {
      kicker: "Rotation interval",
      headline: "Shoulders delay while hips unwind.",
      detail: `The two axes separate by ${separation}; the arc shows stored rotation before the foot releases through the ball.`,
      read: "axis + arc",
      color: HIP_COLOR,
    };
  }
  if (beatId === "ball-path") {
    return {
      kicker: "Shot output",
      headline: "The released ball path checks the mechanics.",
      detail: "After contact, the scene removes the body metrics and follows whether the ball path continues the intended release.",
      read: "release trail",
      color: BALL_PATH_COLOR,
    };
  }
  return {
    kicker: "Approach path",
    headline: "The strike foot path leads into contact.",
    detail: "Before the freeze, only the strike-foot trail is shown so the viewer can see where the swing arrives from.",
    read: "swing trail",
    color: FOOT_PATH_COLOR,
  };
};

const phaseProgressValue = (frame: number) => {
  if (frame < INTRO_FRAMES) return (frame / INTRO_FRAMES) * 0.08;
  return 0.08 + ((frame - INTRO_FRAMES) / CLIP_FRAMES) * 0.92;
};
...
const PlayerSkeleton: React.FC<{player: Player; isShooter: boolean; muted: boolean; tint: string}> = ({player, isShooter, muted, tint}) => {
  ...
  const boneRadius = 0.05;
  const jointRadius = 0.085;
  const haloRadius = 0.098;
  return (
    <group>
      {BODY_CONNECTIONS.map(([from, to]) => {
        const a = player.parts[from];
...
const ContactRings: React.FC<{position: THREE.Vector3}> = ({position}) => (
  <group>
    <mesh position={position} rotation={[Math.PI / 2, 0, 0]}>
      <torusGeometry args={[0.46, 0.026, 14, 48]} />
      <meshBasicMaterial color={CONTACT_COLOR} transparent opacity={0.82} />
    </mesh>
  </group>
);
...
const MeasurementFocusLayer: React.FC<{
  clip: Clip;
  shotFrame: ShotFrame;
  contactFrame: ShotFrame;
  segmentFrame: number;
  ...
  const beats = buildAnnotationBeats({role: clip.role, score: clip.score, features: clip.features});
  const beat = beats.find((item) => item.id === activeBeatId) || beats[0];
  const frameForFrozenMeasurement = activeBeatId === "foot-path" || activeBeatId === "ball-path" ? shotFrame : contactFrame;
  const shooter = (frameForFrozenMeasurement.players || []).find((player) => player.name === clip.shot.player);
  const ball = frameForFrozenMeasurement.ball?.position ? toThree(frameForFrozenMeasurement.ball.position) : null;
  if (!shooter) return null;

  if (beat.id === "contact-gap") {
    const foot = strikeFootPoint(clip, shooter);
    if (!foot || !ball) return null;
    const progress = eased(segmentFrame, PLANT_MEASURE_END + 10, CONTACT_MEASURE_END - 20);
    return (
      <group>
        <AnimatedDistanceBracket
          start={foot}
          end={ball}
          color={CONTACT_COLOR}
          progress={progress}
  ...
  if (beat.id === "plant-base") {
    const plant = plantFootPoint(clip, shooter);
    if (!plant || !ball) return null;
    const start = new THREE.Vector3(plant.x, 0.08, plant.z);
    const end = new THREE.Vector3(ball.x, 0.08, ball.z);
    const progress = eased(segmentFrame, CONTACT_TRIGGER_END + 10, PLANT_MEASURE_END - 20);
    return (
      <group>
        <AnimatedDistanceBracket
          start={start}
          ...
          liftGuide
        />
        <GroundRing point={start} color={MEASURE_COLOR} radius={0.32} opacity={0.9} />
        <GroundRing point={end} color={CONTACT_COLOR} radius={0.38} opacity={0.74} />
      </group>
    );
  }

  if (beat.id === "hip-shoulder") {
    const leftShoulder = shooter.parts.left_shoulder;
    const rightShoulder = shooter.parts.right_shoulder;
    const leftHip = shooter.parts.left_hip;
    const rightHip = shooter.parts.right_hip;
    const pelvis = shooter.parts.pelvis;
    if (!leftShoulder || !rightShoulder || !leftHip || !rightHip || !pelvis) return null;
    const center = toThree(pelvis).add(new THREE.Vector3(0, 0.78, 0));
    const progress = eased(segmentFrame, CONTACT_MEASURE_END + 8, MECHANICS_END - 36);
    return (
      <group>
        <AnimatedMeasurementLine start={toThree(leftShoulder)} end={toThree(rightShoulder)} color={SHOULDER_COLOR} progress={progress} radius={0.042} />
        <AnimatedMeasurementLine start={toThree(leftHip)} end={toThree(rightHip)} color={HIP_COLOR} progress={progress} radius={0.048} />
        <AngleArc
          center={center}
          shoulderA={toThree(leftShoulder)}
          shoulderB={toThree(rightShoulder)}
          hipA={toThree(leftHip)}
          hipB={toThree(rightHip)}
          progress={progress}
        />
      </group>
    );
  ...
  if (beat.id === "foot-path") {
    const foot = strikeFootPoint(clip, shooter);
    if (!foot) return null;
    return (
      <group>
        <FootPathTrail clip={clip} currentFrame={shotFrame.frameNumber} />
        <Endpoint point={foot} color={FOOT_PATH_COLOR} />
      </group>
    );
  }

  const currentBall = shotFrame.ball?.position ? toThree(shotFrame.ball.position) : ball;
  if (!currentBall) return null;
  return (
    <group>
      <BallTrail clip={clip} currentFrame={shotFrame.frameNumber} />
      <Endpoint point={currentBall} color={BALL_PATH_COLOR} />
    </group>
  );
};

const AnimatedDistanceBracket: React.FC<{
  start: THREE.Vector3;
  end: THREE.Vector3;
  color: string;
  progress: number;
  ...
  radius: number;
  liftGuide?: boolean;
}> = ({start, end, color, progress, labelLift, tickSize, radius, liftGuide = false}) => {
  const clamped = Math.max(0, Math.min(1, progress));
  const currentEnd = start.clone().lerp(end, clamped);
  const direction = end.clone().sub(start);
  const flat = new THREE.Vector3(direction.x, 0, direction.z);
  const normal = flat.length() > 0.001
    ? new THREE.Vector3(-flat.z, 0, flat.x).normalize().multiplyScalar(tickSize)
    : new THREE.Vector3(tickSize, 0, 0);
  const guideLift = liftGuide ? Math.max(0.34, labelLift) : 0;
  const guideStart = start.clone().add(new THREE.Vector3(0, guideLift, 0));
  const guideEnd = end.clone().add(new THREE.Vector3(0, guideLift, 0));
  const guideCurrentEnd = guideStart.clone().lerp(guideEnd, clamped);
  const showEnd = clamped > 0.86;
  return (
    <group>
      <MeasurementLine start={start} end={end} color={MUTED_MEASURE_COLOR} radius={radius * 0.42} opacity={0.18} />
      <AnimatedMeasurementLine start={start} end={end} color={color} progress={clamped} radius={radius} />
      {liftGuide ? <MeasurementLine start={start} end={guideStart} color={color} radius={radius * 0.5} opacity={0.74} /> : null}
      {liftGuide && showEnd ? <MeasurementLine start={end} end={guideEnd} color={color} radius={radius * 0.5} opacity={0.74} /> : null}
      {liftGuide ? <MeasurementLine start={guideStart} end={guideCurrentEnd} color={color} radius={radius * 0.9} opacity={0.96} /> : null}
      <MeasurementLine start={start.clone().sub(normal)} end={start.clone().add(normal)} color={color} radius={radius * 0.72} opacity={0.9} />
      {showEnd ? <MeasurementLine start={end.clone().sub(normal)} end={end.clone().add(normal)} color={color} radius={radius * 0.72} opacity={0.9} /> : null}
      <Endpoint point={start} color={color} />
      <Endpoint point={currentEnd} color={color} />
    </group>
  );
};

const AnimatedMeasurementLine: React.FC<{
  start: THREE.Vector3;
  end: THREE.Vector3;
  color: string;
  progress: number;
  ...
  const clamped = Math.max(0, Math.min(1, progress));
  const currentEnd = start.clone().lerp(end, clamped);
  if (clamped <= 0.01) return null;
  return <MeasurementLine start={start} end={currentEnd} color={color} radius={radius} opacity={0.96} />;
};

const MeasurementLine: React.FC<{start: THREE.Vector3; end: THREE.Vector3; color: string; radius?: number; opacity?: number}> = ({
  start,
  end,
  color,
  radius = 0.026,
  ...
  <CylinderBetween start={start} end={end} radius={radius} color={color} opacity={opacity} />
);

const Endpoint: React.FC<{point: THREE.Vector3; color: string}> = ({point, color}) => (
  <mesh position={point}>
...
const GroundRing: React.FC<{point: THREE.Vector3; color: string; radius: number; opacity?: number}> = ({point, color, radius, opacity = 0.9}) => (
  <mesh position={point} rotation={[Math.PI / 2, 0, 0]}>
    <torusGeometry args={[radius, 0.018, 10, 40]} />
...
const AngleArc: React.FC<{
  ...
  hipB: THREE.Vector3;
  progress: number;
}> = ({center, shoulderA, shoulderB, hipA, hipB, progress}) => {
  const points = useMemo(() => {
    const shoulderAngle = axisAngle(shoulderA, shoulderB);
    const hipAngle = axisAngle(hipA, hipB);
    const delta = shortestAngleDelta(hipAngle, shoulderAngle);
    const steps = 32;
    const radius = 0.82;
    const result: THREE.Vector3[] = [];
    const shownSteps = Math.max(1, Math.round(steps * Math.max(0, Math.min(1, progress))));
    for (let index = 0; index <= shownSteps; index += 1) {
      const angle = hipAngle + delta * (index / steps);
  ...
  return <Line points={points} color={HIP_COLOR} opacity={0.96} />;
};

const LabelSprite: React.FC<{position: THREE.Vector3; text: string; color: string}> = ({position, text, color}) => {
  const texture = useMemo(() => {
    ...
    context.fillStyle = "rgba(5, 10, 8, 0.86)";
  ...
  return (
    <sprite position={position} scale={[1.15, 0.32, 1]}>
...
const activeClip = (frame: number) => {
  ...
  return {clip: data.clips[0], segmentFrame: CLIP_FRAMES, phase: "shot result"};
};

const phaseForSegment = (segmentFrame: number) => {
  if (segmentFrame < CONTACT_TRIGGER_END) return "approach to contact";
  if (segmentFrame < PLANT_MEASURE_END) return "plant distance";
  if (segmentFrame < CONTACT_MEASURE_END) return "contact gap";
...
const frameForSegment = (clip: Clip, segmentFrame: number): ShotFrame => {
  ...
  if (segmentFrame < CONTACT_MEASURE_END) return nearestFrame(clip, contact);
  if (segmentFrame < MECHANICS_END) {
    const frameNumber = interpolate(segmentFrame, [CONTACT_MEASURE_END, MECHANICS_END], [Math.max(clip.frameWindow.start, contact - 4), Math.min(clip.frameWindow.end, contact + 10)], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
...
const cameraForSegment = (clip: Clip, segmentFrame: number, shotFrame: ShotFrame, contactFrame: ShotFrame, activeBeatId: string) => {
  const contactTarget = focusTarget(clip, contactFrame);
  const liveTarget = focusTarget(clip, shotFrame);
  const annotationTarget = annotationTargetForBeat(clip, activeBeatId === "foot-path" || activeBeatId === "ball-path" ? shotFrame : contactFrame, activeBeatId);
  ...
  if (segmentFrame < CONTACT_TRIGGER_END) {
    ...
    radius = lerp(8.8, 5.2, progress);
    height = lerp(4.4, 3.0, progress);
    angle = lerp(-0.82, -0.62, progress);
    target = liveTarget.clone().lerp(annotationTarget || contactTarget, progress * 0.78);
  } else if (segmentFrame < CONTACT_MEASURE_END) {
    const progress = eased(segmentFrame, CONTACT_TRIGGER_END, CONTACT_MEASURE_END);
    target = annotationTarget || mechanicsTarget(clip, contactFrame);
    radius = lerp(4.4, 3.15, progress);
    height = lerp(2.45, 1.85, progress);
    angle = lerp(-0.66, -0.6, progress);
  } else if (segmentFrame < MECHANICS_END) {
    const progress = eased(segmentFrame, CONTACT_MEASURE_END, MECHANICS_END);
    target = annotationTarget || mechanicsTarget(clip, shotFrame);
    radius = lerp(3.45, 4.75, progress);
    height = lerp(2.1, 2.9, progress);
    angle = lerp(-0.6, -0.44, progress);
  } else {
    const progress = eased(segmentFrame, MECHANICS_END, CLIP_FRAMES);
    target = (annotationTarget || mechanicsTarget(clip, shotFrame)).lerp(liveTarget, progress * 0.2);
    radius = lerp(5.7, 8.0, progress);
    height = lerp(3.25, 4.05, progress);
    angle = lerp(-0.44, -0.1, progress);
  }
...
const annotationTargetForBeat = (clip: Clip, frame: ShotFrame, activeBeatId: string) => {
  const shooter = (frame.players || []).find((player) => player.name === clip.shot.player);
  const ball = frame.ball?.position ? toThree(frame.ball.position) : null;
  if (!shooter) return ball;
  if (activeBeatId === "contact-gap") {
    const foot = strikeFootPoint(clip, shooter);
    if (foot && ball) return foot.clone().lerp(ball, 0.5).add(new THREE.Vector3(0, 0.18, 0));
  }
  if (activeBeatId === "plant-base") {
    const plant = plantFootPoint(clip, shooter);
    if (plant && ball) return plant.clone().lerp(ball, 0.5);
  }
  if (activeBeatId === "hip-shoulder") {
    const pelvis = shooter.parts.pelvis || shooter.parts.neck;
    return pelvis ? toThree(pelvis).add(new THREE.Vector3(0, 0.6, 0)) : ball;
  }
  if (activeBeatId === "foot-path") return strikeFootPoint(clip, shooter) || ball;
  return ball || focusTarget(clip, frame);
};
...
const strikeFootPoint = (clip: Clip, player: Player) => {
  const strikeFoot = String(clip.features.inferred_foot || "right");
  const point = player.parts[`${strikeFoot}_toe`] || player.parts[`${strikeFoot}_ankle`] || player.parts[`${strikeFoot}_heel`];
  return point ? toThree(point) : null;
};
const plantFootPoint = (clip: Clip, player: Player) => {
  const plantFoot = String(clip.features.plant_foot || "left");
  const point = player.parts[`${plantFoot}_toe`] || player.parts[`${plantFoot}_ankle`] || player.parts[`${plantFoot}_heel`];
  return point ? toThree(point) : null;
};
...
const styles: Record<string, React.CSSProperties> = {
  ...
  title: {marginTop: 6, fontSize: 46, fontWeight: 850, lineHeight: 1.04, letterSpacing: 0},
  ...
  activeMetricDot: {width: 10, height: 10, borderRadius: 10},
  activeMetricValue: {fontSize: 18, fontWeight: 850, color: "#f4f8f3"},
  activeMetricWhy: {fontSize: 14, fontWeight: 700, color: "#aebbb3", textAlign: "right"},
  ...
  metricRail: {position: "absolute", left: 54, bottom: 56, display: "flex", gap: 12, alignItems: "stretch"},
  metricCard: {
    width: 184,
    minHeight: 110,
    padding: "14px 16px",
  ...
  metricValue: {fontSize: 34, lineHeight: 1, fontWeight: 900, letterSpacing: 0},
  ...
  outroTitle: {marginTop: 8, fontSize: 38, fontWeight: 900, lineHeight: 1.05, letterSpacing: 0},
