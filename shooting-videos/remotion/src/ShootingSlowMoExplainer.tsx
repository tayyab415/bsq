...
export const SlowMoMechanicsExplainer = () => {
...
const ExplainerOverlay: React.FC<{clip: Clip; frame: number; phase: string}> = ({clip, frame, phase}) => {
  ...
  const isSlowMo = phase === "slow-motion contact";
  ...
  return (
    <>
      ...
      <div style={styles.storyPanel}>
        ...
        <div style={styles.storySummary}>{isSlowMo ? cue.headline : clip.story.summary}</div>
        <div style={styles.storyText}>{isSlowMo ? cue.detail : "Contact freezes the frame; the next beats follow the shooter in slow motion."}</div>
...
const BodyAxes: React.FC<{clip: Clip; player: Player}> = ({clip, player}) => {
  ...
  const shoulderMid = toThree(leftShoulder).clone().add(toThree(rightShoulder)).multiplyScalar(0.5);
  const hipMid = toThree(leftHip).clone().add(toThree(rightHip)).multiplyScalar(0.5);
  ...
  return (
    <group>
      ...
      <LabelSprite text="shoulder line" position={shoulderMid.clone().add(new THREE.Vector3(0, 0.42, 0))} color={SHOULDER_COLOR} />
      <LabelSprite text="hip line" position={hipMid.clone().add(new THREE.Vector3(0, 0.34, 0))} color={HIP_COLOR} />
      <LabelSprite
        text={`separation ${formatDecimal(separation)} deg`}
        position={shoulderMid.clone().lerp(hipMid, 0.5).add(new THREE.Vector3(0, 0.78, 0))}
        color="#ffffff"
      />
    </group>
  );
...
const PlantLegOverlay: React.FC<{clip: Clip; player: Player}> = ({clip, player}) => {
  ...
  return (
    <group>
      ...
      <LabelSprite text="plant knee" position={kneePoint.clone().add(new THREE.Vector3(0, 0.5, 0))} color={PLANT_COLOR} />
      <LabelSprite text={`plant base ${formatDistance(lateral)}`} position={toePoint.clone().add(new THREE.Vector3(0, 0.55, 0))} color={PLANT_COLOR} />
    </group>
  );
};
...
const LabelSprite: React.FC<{text: string; position: THREE.Vector3; color: string}> = ({text, position, color}) => {
...
const activeClip = (frame: number) => {
  if (frame < 30) return {clip: data.clips[0], segmentFrame: 0, phase: "metric setup"};
  if (frame < 180) return {clip: data.clips[0], segmentFrame: frame - 30, phase: frame < 62 ? "contact trigger" : "slow-motion contact"};
  if (frame < 330) return {clip: data.clips[1], segmentFrame: frame - 180, phase: frame < 212 ? "contact trigger" : "slow-motion contact"};
...
const frameForSegment = (clip: Clip, segmentFrame: number): ShotFrame => {
  const contact = contactNumber(clip);
  if (segmentFrame < 30) {
    const frameNumber = interpolate(segmentFrame, [0, 30], [clip.frameWindow.start, contact], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
      easing: Easing.bezier(0.2, 0.8, 0.2, 1),
    });
    return nearestFrame(clip, Math.round(frameNumber));
  }
  const frameNumber = interpolate(segmentFrame, [30, 149], [contact, clip.frameWindow.end], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
...
const cameraForSegment = (clip: Clip, segmentFrame: number, shotFrame: ShotFrame, contactFrame: ShotFrame) => {
  ...
  if (segmentFrame < 30) {
    const progress = eased(segmentFrame, 0, 30);
    radius = lerp(34, 11, progress);
    height = lerp(15, 4.6, progress);
    angle = lerp(-1.15, -0.72, progress);
    target = liveTarget.clone().lerp(contactTarget, progress * 0.7);
  } else {
    const progress = eased(segmentFrame, 30, 149);
    target = mechanicsTarget(clip, shotFrame);
    radius = lerp(5.8, 7.4, progress);
    height = lerp(2.25, 3.25, progress);
    angle = lerp(-0.72, 0.52, progress);
  }
